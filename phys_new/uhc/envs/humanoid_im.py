...
class HumanoidEnv(mujoco_env.MujocoEnv):
    ...
    def __init__(self, cfg, init_expert, data_specs, mode="train", no_root=False):
        ...
        self.smpl_robot_orig = Robot(cfg.robot_cfg, data_dir="data/smpl")

        # self.smpl_robot_orig.beta = torch.from_numpy(np.array([[0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5]])).float()
    ...
    def generate_xml(self,shape, output_path=R'test_new.xml'):
        beta = np.vstack([shape,shape[None,:]])
        gender = [0]
        obj_info = None
        ...
        self.smpl_robot.write_xml(output_path)

    def reset_robot_from_path(self,path):

        self.reload_sim_model_from_path(path)
        # if obj_info is None: 
        self.weight = mujoco_py.functions.mj_getTotalmass(self.model)
        # else:
    ...
    def load_expert(self, expert_data, reload_robot=True,tran_offset=False):
        ...
        if tran_offset:
            for j,tra in enumerate(trans):
                trans[j] = tra+np.array([0,0,0])
        if self.use_quat:
    ...
    def step(self, a):
        ...
        end = (self.cur_t >= cfg.env_episode_len) or (self.cur_t + self.start_ind >= self.expert["len"] + cfg.env_expert_trail_steps - 0)
        done = fail or end
        # if done:
        #     print("done!!!", fail, end)

        percent = self.cur_t / (self.expert["len"] - 0)
        obs = self.get_obs()
    ...
    def reload_sim_model_from_path(self, path):
        del self.sim
        del self.model
        del self.data
        del self.viewer
