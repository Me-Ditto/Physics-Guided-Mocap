...
if __name__ == "__main__":
    ...
    # sys.argv = ['', '--results=./out/rgb_demo_no_split/results_out',
    # #      '--out=./out/rgb_demo_no_split/viz_out',
    # #      '--viz-prior-frame',
    # #      '--viz-final-only',
    # #      '--viz-render-width=1000',
    # #      '--viz-render-height=1002']
    ...
    from uhc.agents.agent_copycat_zc import AgentCopycat_zc

    # agent = agent_dict[cfg.agent_name](cfg, dtype, device, training=True, checkpoint_epoch=args.epoch)
    agent = AgentCopycat_zc(args, dtype, device, training=True, checkpoint_epoch=args.epoch)
    ...
    # close subprocesses
    for i, parentPipe in enumerate(agent._parent_pipes):
